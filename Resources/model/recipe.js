	...
	if(recipes.length == 0){
		for(var i = 0; i < recipesJson.length; i++) {
			...
			if(recipesJson[i].photos.length == 0){
				photoIds.push(getGlass(recipe.getGlassId()).getPhotoId());
			...
			var components = [];
			for(var j = 0; j < recipesJson[i].components.length; j++){
				var component = new Component();
				components.push(recipesJson[i].photos[j]['@attributes'].id);
			}
			recipe.setPhotoIds(photoIds);
...
Recipe.prototype.setComponents = function (value) {
	this.compontents = value;
}
Recipe.prototype.getComponents = function () {
	return this.components;
}
Recipe.prototype.setVideos = function (value) {
	this.videos = value;
}
Recipe.prototype.getVideos = function () {
	return this.videos;
}
